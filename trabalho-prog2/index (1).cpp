...
void mediaList(tipoAluno *v, int i);
void insertOrdenado(tipoAluno *v, tipoAluno q, int i);

void cadastraAluno(tipoAluno *v, tipoAluno *q, int &i);
...
int main() {
    ...
    do {    
        ...
        // Função de ORDENAÇÃO DOS ALUNOS
...
void insertOrdenado(tipoAluno *v, tipoAluno q, int i) {
    ...
    for (j = i; j > 0 && strcmp(q.nome, v[j-1].nome) < 0; j--) {
        if (strcmp(q.nome, v[j-1].nome) < 0) {
            v[j] = v[j-1];
        }
        else {
            v[j] = q;
...
void mediaList(tipoAluno *v, int i) {
    double n1 = v[i].P1, n2 = v[i].P2;
    // Verifica se a nota da PO do aluno é maior que a P1 ou que a P2
    if (v[i].P1 >= v[i].P2 && v[i].PO >= v[i].P2) {
        n1 = v[i].P1;
        n2 = v[i].PO;
    }
    else if (v[i].P2 > v[i].P1 && v[i].PO > v[i].P1 ) {
        n1 = v[i].PO;
        n2 = v[i].P2;
    }
    
    v[i].media = (0.35 * n1) + (0.35 * n2) + (0.3 * v[i].T);
    
    // Verifica a sitação do estudante
    if (v[i].media >= 6.00) {
        strcpy(v[i].state, "Aprovado");
    }
    else {
        strcpy(v[i].state, "Reprovado");
    }
...
void cadastraAluno(tipoAluno *v, tipoAluno *q, int &i) {
    ...
    printf("Aluno numero %d: ", i);
    
    fflush(stdin);
    scanf("%[^0123456789\t] %d %lf %lf %lf %lf", 
        q[0].nome, q[0].RA, q[0].P1,
        q[0].P2, q[0].T, q[0].PO);

    mediaList(q, i);
    q[0].ordem_cadastro = i + 1;
...
void lerArquivo(tipoAluno *v, int &i) {
    ...
    if (ptrarq == NULL) {
        ...
        fclose(ptrarq);
        printf("\nO Arquivo alunos.txt foi aberto e lido com sucesso!!\nForam cadastrados %d novos alunos.\n", cadastrados);
    }
